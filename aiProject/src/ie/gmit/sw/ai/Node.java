...
public class Node {
	...
	public Node[] children(Node[][] maze) {
		...
		if (col - 1 >= 0 && maze[row][col - 1].getState() != 'X')
			children[0] = maze[row][col - 1]; // A West edge
		if (col + 1 < maze[row].length && maze[row][col + 1].getState() != 'X')
			children[1] = maze[row][col + 1]; // An East Edge
		if (row - 1 >= 0 && maze[row - 1][col].getState() != 'X')
			children[2] = maze[row - 1][col]; // A North edge
		if (row + 1 < maze.length && maze[row + 1][col].getState() != 'X')
